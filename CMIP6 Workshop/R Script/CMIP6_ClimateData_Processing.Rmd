...
📂 Project Root
│── 📂 Inputs Folder
│   │── 📂 Observed_Climate_Variables  # Contains observed PRISM climate data (e.g., precipitation, temperature)
│   │   │── pcp_mm.csv                 # Observed precipitation data (mm)
│   │   │── hmd_percent.csv            # Observed relative humidity data (%)
│   │   │── tmp_max_c.csv              # Observed maximum temperature (°C)
│   │   │── tmp_min_c.csv              # Observed minimum temperature (°C)
│   │   │── wnd_mps.csv                # Observed wind speed (m/s)
│
│── 📂 Outputs Folder
│   │── 📂 raw cmip6 unit converted csv  # Stores raw CMIP6 data converted to appropriate units
│   │── 📂 bias corrected cmip6 csv      # Stores bias-corrected CMIP6 data using Random Forest
│   │── 📂 Statistical_Metrics           # Stores computed statistical metrics comparing raw, corrected, and observed data
│   │── 📂 CDF_Plots                     # Stores CDF plots for visualization of distributions
│
│── 📂 Scripts
│   │── data_download.R                    # Script for downloading and processing CMIP6 data
│   │── bias_correction_RF.R               # Script for bias correction using Random Forest
│   │── statistical_analysis.R             # Script for computing performance metrics
│   │── visualization_CDF.R                # Script for plotting CDF curves
│
│── CMIP6_Data_Extraction_and_Bias_Correction.Rmd   # This tutorial notebook
│── README.md                                       # Documentation on how to use the workflow
```
---
...
At the end of this tutorial, you will be able to: 
    ...
    ✔ Retrieve CMIP6 climate model projections for your desired locations.
    ✔ Apply bias correction using Random Forest Regression.
    ✔ Evaluate the improvements in climate projections using statistical metrics.
    ✔ Visualize Cumulative Distribution Functions (CDFs) for better understanding of data distributions.
...
```plaintext
📂 Inputs Folder
│── 📂 NEX-GDDP-CMIP6                       # Folder containing CMIP6 NetCDF files
│   │── 📂 ACCESS-CM2                       # Example GCM (Climate Model)
│   │   │── 📂 historical                   # Scenario Folder (historical or future projections)
│   │   │   │── 📂 pr                       # Variable Folder (precipitation)
│   │   │   │   │── filtered_data.csv       # CSV listing available NetCDF files
│   │   │   │   │── <NetCDF Files>          # NetCDF files for precipitation
│   │   │   │── 📂 hurs                     # Variable Folder (relative humidity)
│   │   │   │   │── filtered_data.csv
│   │   │   │   │── <NetCDF Files>
│
📂 Outputs Folder
│── 📂 CMIP6 Data extracted from netcdfs     # Stores extracted climate data in CSV format
│── 📂 Temporary NetCDF file                 # Stores downloaded NetCDF files temporarily
│
📂 R Script Folder
│── data_download.R                           # This script for extracting CMIP6 data
│── bias_correction_RF.R                      # Bias correction using Random Forest
```
...
- search_folder → The directory where raw CMIP6 NetCDF files are stored.
- output_folder → The location where extracted CSV files (processed station data) will be saved.
- specified_temp_folder → A temporary directory used to store downloaded NetCDF files.
- locations_file → The CSV file containing station locations (latitude, longitude, and ID).
...
- gcm_list → Contains the list of General Circulation Models (GCMs) to be processed. The example uses ACCESS-CM2.
- scenario_list → Specifies the climate projection scenarios:
...
- variable_list → Specifies the climate variables to extract:
  - "pr" → Precipitation
  - "hurs" → Relative Humidity
- delete_NetCDF_file → Controls whether the downloaded NetCDF files are deleted after processing.
...
#---------------------------------------------------------------------------------------------------#
# 2.1. Defining Base Directories
search_folder <- here("..", "..", "NEX-GDDP-CMIP6-Full")
...
# 3. Selecting GCMs, Scenarios, and Variables
gcm_list <- c("ACCESS-CM2") # List of climate models to process
scenario_list <- c("historical","ssp126","ssp245","ssp370") # CMIP6 scenarios (e.g., historical, ssp245, ssp370)
variable_list <- c("pr","hurs","tasmax") # Climate variables to extract (daily precipitation, relative humidity, maximum daily temperature)
...
```plaintext
📂 Outputs folder
│── 📂 CMIP6 Data extracted from netcdfs      # Folder containing extracted CSVs
│   │── 📂 ACCESS-CM2                          # GCM folder
│   │   │── 📂 historical                      # Scenario folder
│   │   │   │── 📂 pr                          # Variable folder (Precipitation)
│   │   │   │   │── ACCESS-CM2_historical_pr_2000.csv
│   │   │   │   │── ACCESS-CM2_historical_pr_2001.csv
│   │   │   │   │── ...
│   │   │   │── 📂 hurs                        # Variable folder (Relative Humidity)
│   │   │   │   │── ACCESS-CM2_historical_hurs_2000.csv
│   │   │   │   │── ACCESS-CM2_historical_hurs_2001.csv
│
📂 Outputs folder
│── 📂 raw cmip6 csv                           # Processed (merged) CSVs
│   │── 📂 ACCESS-CM2                          # GCM folder
│   │   │── ACCESS-CM2_historical_pr_2000-2010.csv
│   │   │── ACCESS-CM2_historical_hurs_2000-2010.csv
│
📂 R Script Folder
│── cmip6_data_merge.R                         # This script for merging CSVs
```
...
- main_dir → Directory containing the yearly extracted CMIP6 station data.
- output_dir → Directory where the merged, stacked CSVs will be saved.
...
- list.dirs(main_dir, recursive = FALSE) → Finds all available GCM folders.
...
```plaintext
📂 Outputs folder
│── 📂 raw cmip6 csv                     # Raw data (before conversion)
│   │── 📂 ACCESS-CM2                     # GCM folder
│   │   │── ACCESS-CM2_historical_pr_2000-2010.csv
│   │   │── ACCESS-CM2_historical_hurs_2000-2010.csv
│   │   │── ACCESS-CM2_historical_tasmax_2000-2010.csv
│   │   │── ACCESS-CM2_historical_tasmin_2000-2010.csv
│   │   │── ACCESS-CM2_historical_sfcWind_2000-2010.csv
│
📂 R Script Folder
│── unit_conversion.R                    # This script for conversion
```
...
```Plaintext
📂 Outputs folder
│── 📂 raw cmip6 unit converted csv       # Processed (unit-converted) data
│   │── 📂 pr                              # Precipitation folder
│   │   │── ACCESS-CM2_historical_pr_2000-2010.csv
│   │── 📂 hurs                            # Relative Humidity folder
│   │   │── ACCESS-CM2_historical_hurs_2000-2010.csv
│   │── 📂 tasmax                          # Maximum Temperature folder
│   │   │── ACCESS-CM2_historical_tasmax_2000-2010.csv
│   │── 📂 tasmin                          # Minimum Temperature folder
│   │   │── ACCESS-CM2_historical_tasmin_2000-2010.csv
│   │── 📂 sfcWind                         # Wind Speed folder
│   │   │── ACCESS-CM2_historical_sfcWind_2000-2010.csv
```
...
- data.table → Efficient data handling and processing.
- readr → For reading and writing CSV files.
- stringr → Used for string operations such as identifying variable names.

**2.2. Define Input and Output Directories**

- input_folder → Folder containing raw CMIP6 CSVs.
- output_folder → Folder where unit-converted CSVs will be saved.
...
- list.dirs(input_folder, full.names = TRUE, recursive = FALSE) → Retrieves available GCM folders.
...
- Precipitation (pr): Converts kg/m²/s → mm/day by multiplying values by 86400.
- Temperature (tasmax, tasmin): Converts Kelvin → Celsius by subtracting 273.15.
...
for (gcm_folder in gcm_folders) {
  ...
  for (file_path in csv_files) {
    ...
    if (variable == "pr") {
      cat(sprintf("Processing precipitation file: %s (Converting kg/m²/s to mm/day)\n", file_name))
...
```Plaintext
📂 Outputs folder
│── 📂 raw cmip6 unit converted csv          # Unit-converted CMIP6 data
│   │── 📂 pr                                 # Precipitation
│   │   │── ACCESS-CM2_historical_pr_2000-2010.csv
│   │   │── ACCESS-CM2_ssp245_pr_2020-2030.csv
...
│
📂 Inputs Folder
│── 📂 Observed_Climate_Variables            # PRISM Observed Data
│   │── pcp_mm.csv
│   │── tmp_max_c.csv
│   │── tmp_min_c.csv
│   │── hmd_percent.csv
...
│
📂 R Script Folder
│── bias_correction.R                        # This script

*After Bias Correction (Output Directory)*

📂 Outputs folder
│── 📂 bias corrected cmip6 csv              # Bias-corrected CMIP6 data
│   │── 📂 pr                                 # Precipitation
│   │   │── ACCESS-CM2_historical_RFcorrected_pr_2000-2010.csv
│   │   │── ACCESS-CM2_ssp245_RFcorrected_pr_2020-2030.csv
│   │── 📂 tasmax                             # Max Temperature
│   │── 📂 tasmin                             # Min Temperature
│   │── 📂 hurs                               # Relative Humidity
│   │── 📂 sfcWind                            # Wind Speed
...
- randomForest → Implements the Random Forest Regression Model for bias correction.
- readr → Reads CMIP6 and observed data CSV files.
- dplyr → Handles data manipulation.
- stringr → Extracts GCM names, scenarios, and year ranges from file names.

**2.2. Define Paths for Input and Output Data**

- cmip6_base_folder → Contains unit-converted CMIP6 files (input data).
- output_base_folder → Stores bias-corrected output data.
...
**Summary**
    ...
    ✔ Trains Random Forest on historical CMIP6 vs. observed data.
    ✔ Applies trained correction to future projections.
    ✔ Handles missing files dynamically.
    ✔ Saves corrected data for all variables, GCMs, and scenarios. 
...
for (variable in names(variables)) {
  ...
  for (gcm_name in gcm_names) {
    print(paste("🔹 Processing GCM:", gcm_name, "for variable:", variable))
...
📂 Inputs Folder
│── 📂 Observed_Climate_Variables              # PRISM Observed Data
│   │── pcp_mm.csv                             # Precipitation (mm)
│   │── tmp_max_c.csv                          # Max Temperature (°C)
│   │── tmp_min_c.csv                          # Min Temperature (°C)
│   │── hmd_percent.csv                        # Relative Humidity (%)
│   │── wnd_mps.csv                            # Wind Speed (m/s)
│
📂 Outputs folder
│── 📂 raw cmip6 unit converted csv           # Unit-converted raw CMIP6 data
│   │── 📂 pr
│   │── 📂 hurs
│   │── 📂 tasmax
│   │── 📂 tasmin
...
│
📂 R Script Folder
│── bias_correction_metrics.R                 # This script
```
...
```plaintext
📂 Outputs folder
│── 📂 Bias_Correction_Metrics                # Folder where metrics will be saved
│   │── Bias_Correction_Metrics_pr.csv
│   │── Bias_Correction_Metrics_hurs.csv
│   │── Bias_Correction_Metrics_tasmax.csv
│   │── Bias_Correction_Metrics_tasmin.csv
│   │── Bias_Correction_Metrics_sfcWind.csv
```
...
- observed_folder → Contains PRISM observed climate data.
- raw_cmip6_folder → Contains unit-converted raw CMIP6 data.
- bias_corrected_folder → Stores bias-corrected CMIP6 data.
- metrics_output_base → Defines where the evaluation results will be saved.
...
**Summary**
    ...
    ✔ Calculates performance metrics (RMSE, NSE, KGE, etc.).
    ✔ Ensures data is matched by station and date.
    ✔ Handles missing files dynamically.
    ✔ Saves results to separate CSV files for each variable. 
...
│
📂 Outputs folder
│── 📂 raw cmip6 unit converted csv  # CMIP6 raw data (converted to correct units)
│   │── 📂 pr
│   │── 📂 hurs
│   │── 📂 tasmax
│   │── 📂 tasmin
...
│
📂 R Script Folder
│── plot_bias_correction_statistics.R # This script
```
...
- GCM_Name → The General Circulation Model (GCM) to analyze.
- Station → The station for which data should be plotted.

**2.3. Define File Paths**

- base_raw_data_folder → Contains unit-converted CMIP6 raw data.
- base_bias_corrected_folder → Contains bias-corrected CMIP6 data.
- observed_data_folder → Contains PRISM observed datasets.
...
**Summary**
    ...
    ✔ Checks for missing files dynamically.
    ✔ Computes CDFs for Raw, Bias-Corrected, and Observed data.
    ✔ Plots CDFs for station-specific comparisons.
    ✔ Ensures better visualization of bias correction performance. 
...
GCM_Name <- "ACCESS-CM2"   # Change this to your required GCM
...
# Define constants
GCM_Name <- "ACCESS-CM2" #Change this to your desired GCM
Station <- "DSM" # Change this to your desired location
